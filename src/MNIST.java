...
/**
 * Title: MNIST
 * Description: This program implements the K-NN (K-Nearest Neighbors) algorithm
 * using the MNIST dataset to classify handwritten digit images.
 *
 * @author Krish Prasad
 * @since ${05-22-2023}
 */
public class MNIST {
    ...
    private static class ImageLabel implements Comparable<ImageLabel> {
        ...
        public int compareTo(ImageLabel imageLabel) {
            if (priority < imageLabel.priority) {
                return -1;
            } else if (priority > imageLabel.priority) {
                return 1;
            } else {
                return 0;
            }
    ...
    public static float totalDist(float[] img1, float[] img2) throws IllegalArgumentException {
        ...
        for (int i = 0; i < img1.length; i++) {
            totalDiff += Math.pow((img1[i] - img2[i]), 2);
        }
    ...
    public static int predict(float[] image, int k) {
        ...
        MyPriorityQueue<ImageLabel> pq = new MyPriorityQueue<>(k);

        for (int i = 0; i < NUM_TRAIN; i++) {
            float priority = totalDist(image, TRAIN_IMAGES[i]);
            pq.offer(new ImageLabel(TRAIN_LABELS[i], priority));
        }

        int[] countingArray = new int[NUM_CLASSES];
        for (int i = 0; i < k; i++) {
            ImageLabel lab = pq.poll();
            countingArray[lab.label]++;
        }

        int mxCount = 0;
        int maxLab = -1;
        for (int i = 0; i < NUM_CLASSES; i++) {
            if (countingArray[i] > mxCount) {
                maxLab = i;
                mxCount = countingArray[i];
            }
        }
    ...
    public static void downloadMNIST() {
        /**
         * Custom class for storing both labels and priority values in the priority queue.
         */
